...
def login_view(request):
    ...
    if request.method == 'POST':
        ...
        # else: form is invalid, render template with errors
...
def register_admin_view(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(request,'Admin account created for {0} !!!'.format(username),extra_tags = 'alert alert-success alert-dismissible show' )
    ...
    dataset = dict()
    dataset['form'] = form
    dataset['title'] = 'register admin'
    return render(request,'admin/register.html',dataset)
